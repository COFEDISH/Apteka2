...
class MainWindow(QMainWindow):
    ...
    def populate_drug_table(self, data):
        ...
        for row, item in enumerate(data):
            self.drug_table.setItem(row, 0, QTableWidgetItem(item.get('Категория по действию')))
            self.drug_table.setItem(row, 1, QTableWidgetItem(item.get('Категория по системе')))
    ...
    def populate_pharmacy_table(self, data):
        ...
        for row, item in enumerate(data):
            ...
            self.pharmacy_table.setItem(row, 1, QTableWidgetItem(item.get('Название')))
            self.pharmacy_table.setItem(row, 2, QTableWidgetItem(item.get('Адрес')))
    ...
    def add_record(self):
        add_type_dialog = AddRecordTypeDialog(self.drug_table, self.pharmacy_table, self.city)
